...
QTextStream& operator>>(QTextStream& stream, PostBaseIterativeValuesContainer::BaseContainer& container)
{
	...
	QStringList names, ids, types;
	names = stream.readLine().split(",");
	ids = stream.readLine().split(",");
	types = stream.readLine().split(",");

	for (int i = 0; i < names.size(); ++i) {
		QString name = names.at(i);
		int arrayId = ids.at(i).toInt();
		QString type = types.at(i);

		if (type == "int") {
			auto c = new PostBaseIterativeValuesContainer::IntegerValueContainer(iRIC::toStr(name), arrayId);
			container.addContainer(c);
		} else if (type == "real") {
			auto c = new PostBaseIterativeValuesContainer::RealValueContainer(iRIC::toStr(name), arrayId);
			container.addContainer(c);
		}
	}
...
QTextStream& operator<<(QTextStream& stream, PostBaseIterativeValuesContainer::BaseContainer& container)
{
	...
	vals.clear();
	for (auto c : container.integerContainers()) {
		vals.push_back(QString::number(c->arrayId()));
	}
	for (auto c : container.realContainers()) {
		vals.push_back(QString::number(c->arrayId()));
	}
